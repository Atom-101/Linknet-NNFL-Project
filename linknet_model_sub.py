...
class DecoderBlock(nn.Module):
    def __init__(self,m,n,up=True):
        super().__init__()
        if up:
            self.layers = nn.Sequential(nn.Conv2d(m,4*m,1,bias=False),
                                    nn.BatchNorm2d(4*m),
                                    nn.ReLU(inplace=True),
                                    nn.PixelShuffle(2),
                                    nn.Conv2d(m,n,1,bias=False),
                                    nn.BatchNorm2d(n),
                                    nn.ReLU(inplace=True))
        else:
            self.layers = nn.Sequential(nn.Conv2d(m,m//4,3,bias=False),
                                    nn.BatchNorm2d(m//4),
                                    nn.ReLU(inplace=True),
                                    nn.Conv2d(m//4,n,3,bias=False),
                                    nn.BatchNorm2d(n),
                                    nn.ReLU(inplace=True))
    def forward(self,x):
